...
Figure 2 in the Appendix is a boxplot of the comparison of the mpg between manual and automatic transmission, showing that over manual has higher mpg than automatic.  Figure 3 shows the relationship between the variable in the dataset.  This assists with selection of model variables.
...
The coefficients for model 4 indicate automatic transmission cars average 9.6 miles per gallon and manual cars delivering an additional 2.9 mpg.  The adjusted R-squared indicates a 83% fit with the variance and the model's p-value is just less than the 5% cut off.  See Figure 6 & 7 for more detail.
...
boxplot(mpg ~ am, data = mtcars, main = "Fig 2. MPG by Transmission Type", ylab = "MPG")
...
ggpairs(
 ...
 title = "Fig 3. Pair Relationships"
)
```
